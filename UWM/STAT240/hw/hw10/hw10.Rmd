...
> We observe that the distribution is not normally ndistributed. It is also very strongly skewed towards the right. There are many observations around 45 and 65 resulting in two peaks. Observations are as likely to be high as low.
...
> We are 95% sure that on average April 14th will have the temperature between 50.384 and 65.415.
...
The distribution has the following n, mean and sd.
```{r}
...
Null Hypothesis: mu = 55.6
Alternate Hypothesis: mu != 55.6
...
> As we observe, the p-value is 0.735, which is considerably greater than than the standard alpha = 0.05. Due to this there is no satisfactory evidence that the null hypothesis can be rejected. Thus it is likely that the mu is equal to 55.6.
...
> We observe that the value of p-test is very high and considerably greater than the standard alpha value of 0.05. So there is no evidence to reject the null hypothesis. Thus the true difference in means of the two periods is likely to be 0, i.e. the average latest day of atleast 1 inch of snow is likely the same.
...
> We can be 95% sure that the true mean of finishing times for men aged 35-39 around the wourld would be between 223.2420 and 224.7694 mins.
...
Model: 
Xi∼F1(m1,σ1),i=1,…,n_m_10
 
Yi∼F2(m2,σ2),i=1,…,n_m_11

Null Hypothesis: m1=m2 or m1 - m2 = 0
Alternate Hypothesis: m1!=m2 or m1 - m2 != 0
...
> From the observed p-value we can observe that it is greater than the standard alpha of 0.05 and thus we have no considerable evidence to negate the null hypothesis. Thus the average 35-39 year old male does not have different times in 2010 and 2011.
