...
In approximately 16 winters was the total duration
where Lake Monona was at least 50% covered with ice between 40 to 70 days.
...
ggplot(monona, aes(x = duration)) +
  geom_histogram(boundary = 70, binwidth = 5, 
                 fill = "cyan", 
                 color = "forestgreen") + 
    ggtitle("No of Winters Lake Monona was atleast 50% covered with ice for certain number of days") +
...
As we observe, the scatter plots the data values showing the correspondence between duration and year1 on the graph. The line of code adds a smooth curve showing the overall trend of correspondence of the data. It reduces the noise of the data greatly and give the average or expected duration for the given year.
...
As we observe, the slope of the curve is negative and thus the general trend of data is decreasing, i.e. the average duration for which the lake has been atleast 50% covered with ice has been decreasing.
...
In a typical year near 1875, the lake was on average closed for around 118 to 120 days.
...
In a typical year near 2000, the lake was on average closed for around 90 days.
...
ggplot(monona, aes(y=duration)) + 
  geom_boxplot(fill = "yellow",
               color = "magenta") + 
  ylab("Duration of days when Lake was closed") + 
  ggtitle("Distribution of the number of days Lake Monona is closed by ice")
```
...
Approximately 92 days is the median number of days for which the Lake Monona was closed with ice.
...
ggplot(monona, aes(x = ff_cat)) +
  ...
  ggtitle("Number of Winters when the first freeze occured in each half-month period")
...
###Response
The bar plot is used when the count or bar height for a categorical variable has to be automatically determined based off of the given columns of the Dataframe. However, the Column plot is used when the count for each category is already available in the Data and is needed to be passed into the function.
...
ggplot(monona, aes(x = ff_x, y = duration, color = period50)) +
  ...
  xlab("First freeze date as no. of days since June 30 in period") +
  ylab("Duration for which lake is closed") +
  ggtitle("Duration for which lake Monona and the first freeze date is closed in different 50 year periods") +
...
### Response
Based on the graph if the winter was in 1870s vs the present then the prediction would change from 88 to 82 days.
...
### Response
Faceting makes it easier to compare the relationship between the first date of closing with ice and total duration of the freeze among the different 50 year time periods since the data is less cluttered contributing to the clarity of analysis.