...
co2 = co2 %>%
  drop_na(ppm) %>%
  mutate(date = ymd(str_c(year,month,day,sep="-")), month = month(date, label=TRUE)) %>%
  select(date, year.decimal, year, month, ppm)

sea = sea %>%
  mutate(date = as_date(date_decimal(year.decimal)), year = year(date), month = month(date, label=TRUE)) %>%
  pivot_longer(names_to = "method", values_to="mm", cols = c(Jason1, Jason2, Jason3, TOPEX.Poseidon)) %>%
  drop_na(mm)
...
co2_data = no_data_co2 %>%
  pivot_wider(names_from = month, values_from = n, names_sort=TRUE)
sea_data = no_data_sea %>%
  pivot_wider(names_from = month, values_from = n, names_sort=TRUE)
...
```{r}
co2 = co2 %>%
  filter(year != 1974 & year != 1975 & year != 2023)

sea = sea %>%
  filter(year != 2022 & year != 1992)

ggplot(co2, aes(x=date, y=ppm)) +
  geom_line() + 
  geom_smooth(se=FALSE) +
  xlab("Year") +
  ylab("Concentration of CO2 (ppm)") +
  ggtitle("Relationship between Concentration of CO2 in the atmosphere(ppm) and time")
ggplot(sea, aes(x=date, y=mm)) +
  geom_line() + 
  geom_smooth(se=FALSE) +
  xlab("Year") +
  ylab("Sea Level (mm)") +
  ggtitle("Relationship between Sea Level(mm) and time")
```


> An interesting thing about both the plots would be that although both show an overall increasing trend throguhout the years, they also display a periodic pattern of rising and falling which might be attributed as seasonal changes. It can also be noted that although the sea levels have risen with the increases in CO2 concentration in the atmosphere, CO2 shows almost a constant overall trend of growth, whereas, sea levels have risen with varying rates, slowing down considerably around 2005-2010, increasing rapidly from then on with varying rate of growths through the 2012-2015 and 2015-2023 periods.
...
sea_avg = sea %>%
  group_by(year) %>%
  summarize(avg_mm = mean(mm))

joined_data = inner_join(co2_avg, sea_avg)

ggplot(joined_data, aes(x=avg_mm, y=avg_ppm)) +
  geom_point() +
  geom_smooth(se=FALSE) +
  xlab("Sea Level (mm)") +
  ylab("Concentration of CO2 (ppm)") +
  ggtitle("Relation between Concentration of CO2 and Sea Level by years")
```

> We observe that the overall trend suggests that as the concentration of CO2 has risen every year,the sea level has risen correspondingly evry year. This suggests a strong positive correlation between the increase in CO2 levels and the sea level.
...
names = names %>%
  group_by(year, sex) %>%
  mutate(sex_pop = sum(frequency)) %>%
  group_by(year, sex, name) %>%
  mutate(prop.by.year.sex = sum(frequency)/sex_pop) %>%
  ungroup() %>%
  group_by(sex, name) %>%
  mutate(total.by.name.sex = sum(frequency))
```
...
req = names %>%
  group_by(year, sex) %>%
  slice_max(n=1, order_by = frequency) %>%
...
req2 = req %>%
  pivot_longer(names_to = "Gender", cols = c(F, M))

q = names %>%
  semi_join(req2, by=c("name"="value", "sex"="Gender"))

ggplot(q, aes(x=year, y=prop.by.year.sex)) +
  geom_col() +
  facet_wrap(sex~name,ncol=4) +
  xlab("Year") +
  ylab("Proportion of name by Sex") +
  ggtitle("Rpoportion of name ")
```
