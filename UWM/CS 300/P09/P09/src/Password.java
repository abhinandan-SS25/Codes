...
//
// Title: Methods to model the Password class
...
public class Password {
  ...
  private static float computeStrengthRating(String s) {
    int charSet = 0;
    // if you want you can read up on regular expressions to decipher the strings I
    // used
  ...
  public int compareTo(Password other, Attribute a) {
    // compare on attribute occurences
    if (a == Attribute.OCCURRENCE) {
      return this.occurrences - other.occurrences;
    }
    // compare on attribute strength_rating
    ...
    // compare on hashed_password using the string compareTo
    else {
